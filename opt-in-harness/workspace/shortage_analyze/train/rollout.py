...
OPT_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_WORKSPACE_ROOT = Path(__file__).resolve().parents[1]
COMMON_TRAIN_ROOT = OPT_ROOT / "train"
...
DEFAULT_INITIAL_SKILL = DEFAULT_WORKSPACE_ROOT / "init-skill" / "initial_skill.md"
DEFAULT_INITIAL_SCRIPT = DEFAULT_WORKSPACE_ROOT / "init-skill" / "analyze_shortage.py"
PROMPTS_DIR = DEFAULT_WORKSPACE_ROOT / "train" / "prompts"
NONE_LABEL = "- 未匹配到分支"
...
def _build_codegen_prompt(skill_content: str, reference_script: str) -> str:
    template = (PROMPTS_DIR / "script_codegen.md").read_text(encoding="utf-8-sig")
    return (
        template
        .replace("__SKILL_CONTENT__", skill_content)
        .replace("__REFERENCE_SCRIPT__", reference_script)
    )
...
def run_batch(
    *,
    ...
    codegen_model = (
        script_codegen_model
        or os.environ.get("OPT_IN_HARNESS_AGENT_MODEL")
        or os.environ.get("SHORTAGE_ANALYZE_AGENT_MODEL")
        or "harness-default"
    )
