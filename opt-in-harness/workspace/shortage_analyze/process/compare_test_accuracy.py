...
DEFAULT_SPLIT = DEFAULT_WORKSPACE_ROOT / "data" / "shortage_analyze_split" / "test" / "items.json"
DEFAULT_PRIVATE_LABELS = (
    DEFAULT_WORKSPACE_ROOT / "data" / "shortage_analyze_split" / "_private" / "test_labels.json"
)
DEFAULT_SOURCE_EXCEL = Path(r"D:\code\github\hehe03\data.xlsx")
FALLBACK_SOURCE_EXCEL = DEFAULT_WORKSPACE_ROOT / "data" / "data.xlsx"
DEFAULT_LEGACY_SCRIPT = DEFAULT_WORKSPACE_ROOT / "init-skill" / "analyze_shortage.py"
DEFAULT_OPTIMIZED_SCRIPT = (
    DEFAULT_WORKSPACE_ROOT / "outputs" / "optimized" / "best" / "scripts" / "analyze_shortage.py"
)
DEFAULT_OUTPUT = DEFAULT_WORKSPACE_ROOT / "data" / "eval" / "test_accuracy_compare.json"
...
def load_gold_labels(args: argparse.Namespace, items: list[dict[str, Any]]) -> tuple[dict[str, str], str]:
    ...
    raise FileNotFoundError(
        ...
        "可选做法：\n"
        "1. 将原始 data.xlsx 放回 opt-in-harness/workspace/shortage_analyze/data/data.xlsx 后重跑本脚本；或\n"
        "2. 优化结束后运行 prepare_skillopt_data.py --write-test-labels 生成 "
        "opt-in-harness/workspace/shortage_analyze/data/shortage_analyze_split/_private/test_labels.json，"
